# Sermon Short v5 — Visual Identity + Motion Graphics (A Church London)
...
## v5 Motion Additions (선각자 모드 — student-kit 1:1)

| Scene | Motion Partial | 모방 출처 | 내용 |
|-------|---------------|----------|------|
| Hook | `kinetic-type` | linear-promo `01-problem-type` | 단어별 x-cascade stagger (expo.out) + whip-streak |
| Scripture | `card-glow` | aisoc-app-release `scene3-reveal` | 좌측 border draw + gold glow pulse (2 cycles, sine.inOut) |
| Body | `word-highlight` | may-shorts-19 `scene2-rejection` | emphasis 단어 back.out(2.2) flash + 주기적 textShadow pulse |
| Austerity | vignette-deepen (직접) | linear-promo vignette | radial vignette 0→0.7 yoyo + contrast 1→1.5 |
| Outro | `archetype-reveal` | may-shorts-19 `scene7-cta` | scan line sweep + archetype bounce pulse |
| Hook→Scripture | whip-fade shader | `@hyperframes/shader-transitions` | data-transition="whip-fade" at t=3.8s, 0.4s |

**Component 파일**: `compositions/components/` 4개 partial, `window.__motionPartials["name"]` export 패턴.
